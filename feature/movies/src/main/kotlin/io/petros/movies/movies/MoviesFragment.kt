...
class MoviesFragment : MviFragment<MoviesIntent, MoviesState, MoviesSideEffect, MoviesViewModel>(R.layout.movies_fragment),
    ...
    /**
     * The below code snipped would have worked if the 'app:destination' of the 'moviesFragment' fragment would have pointed
     * to an 'android:id' within the same navigation graph. However, due to the fact that the 'app:destination' point to
     * another graph (due to the modularized nature of this project), the autogenerated 'MoviesFragmentDirections' class
     * ends up with an 'ActionOnlyNavDirections' instead, which doesn't hold a 'Movie' argument.
     *
     * 'val action = MoviesFragmentDirections.toMovieDetails(movie)
     *  findNavController().navigate(action)'
     */
    override fun onClick(movie: Movie) {
        val bundle = MovieDetailsFragmentArgs(movie).toBundle()
        findNavController().navigate(R.id.movieDetailsNavigation, bundle)
    }
